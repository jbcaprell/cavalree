defmodule Masonree.BlockTest do
  ...
  describe "Block" do
    test "asks every block for what use defines, and only some for the rest" do
      callbacks = Block.behaviour_info(:callbacks)
      optional = Block.behaviour_info(:optional_callbacks)
      minimal = __MODULE__.Minimal

      defined? = fn {name, arity} ->
        Code.ensure_loaded?(minimal) and
          function_exported?(minimal, name, arity)
      end

      assert Enum.all?(callbacks -- optional, defined?)
      refute Enum.any?(optional, defined?)
    end
  end

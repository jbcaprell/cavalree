defmodule Masonree.Node do
  ...
  The refusal is never rescued, and it reaches all the way down: a child at any
  depth that is not an envelope makes the whole read `:error`. Half a tree is
  not a tolerant answer, it is a lost one, and a caller handed one has no way to
  know what is missing.

  Reading and refusing are one walk. A separate check would descend the whole
  tree to answer a boolean that the read discovers on its way past, so every
  document coming out of the database would be walked twice to be read once.
  This is the path a stored page arrives by, and there is no other.
  ...
  def from_map(%{"type" => type} = serialized) when is_binary(type) do
    case read_children(serialized) do
      {:ok, children} -> {:ok, build(serialized, children)}
      :error -> :error
    end
  end
  ...
  diff be read. Tolerance is the reader’s business; the writer states the whole
  node.
  ...
  @spec build(serialized(), children()) :: t()
  defp build(%{"type" => type} = serialized, children) do
    %__MODULE__{
  ...
  @spec read_children(serialized()) :: :error | {:ok, children()}
  defp read_children(serialized) do
    read = fn child, {:ok, acc} ->
      case from_map(child) do
        {:ok, node} -> {:cont, {:ok, [node | acc]}}
        :error -> {:halt, :error}
      end
    end

    children = take_children(serialized)

    case Enum.reduce_while(children, {:ok, []}, read) do

defmodule Masonree.Conformance do
  ...
  finding is one function over a node and the manifest that declares its
  ...
  is. Findings carry the node’s id and whatever the document cannot already
  supply — the id finds the node, the node knows its type, so an unknown block
  needs no second element where an attribute finding names its key.
  ...
  @doc """
  Returns every finding in `document`, in document order, against `manifests`.
  ...
  @doc """
  Returns a finding for each child of `node` that `manifest` refuses.
  ...
  @doc """
  Returns the finding where `node`’s children breach `manifest`’s bounds.
  ...
  @doc """
  Returns a finding for each key `node` holds that `manifest` never declared.
  ...
  changed — and every such key is named in its own finding. What to do about one
  is not answered here: a report changes nothing. The keys report in the order
  the node’s attribute map iterates, which above 32 keys is not the order they
  were written in.
  ...
  @doc """
  Returns a finding for each key `manifest` requires that `node` omits.
  ...
  @doc """
  Returns every finding at the top of `document`, by `containment`.
  ...
  @doc """
  Returns a finding for each value `node` holds that `manifest` refuses.

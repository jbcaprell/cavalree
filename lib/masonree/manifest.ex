defmodule Masonree.Manifest do
  ...
  manifest alone. Nothing here needs a second block, a stored page or a running
  editor, so the judgment can run where the declaration is written. A name
  ...
  @doc """
  Returns whether the block `manifest` declares has an interior none can start.
  ...
  def validate(manifest) when is_struct(manifest, __MODULE__) do
    reports = [validate_declaration(manifest), validate_interior(manifest)]
  ...
  @doc """
  Returns the rejection where `manifest` offers a template its rule refuses.
  ...
  than through `Masonree.Manifest.Containment.admits?/2`, whose guard refuses
  the non-string type that `validate_templates/1` reports on its own. A block
  with no containment offers no template, and nothing is judged.
  ...
  @doc """
  Returns the rejection where `manifest`’s bounds contradict themselves.
  ...
  @doc """
  Returns a rejection for each of `manifest`’s enums with a refused default.
  ...
  @doc """
  Returns a rejection for each of `manifest`’s enums that repeats a value.
  ...
  @doc """
  Returns a rejection for each of `manifest`’s enums declared with no values.
  ...
  @doc """
  Returns the rejection where `manifest` declares an interior nothing may fill.
  ...
  @doc """
  Returns a rejection for each of `manifest`’s keys of a shape no key may have.
  ...
  @doc """
  Returns the rejection where any of `manifest`’s keys is not a string.
  ...
  @doc """
  Returns a rejection for each required attribute of `manifest` with a default.
  ...
  @doc """
  Returns a rejection for each of `manifest`’s attributes with no declared role.
  ...
  @doc """
  Returns a rejection for each of `manifest`’s scalars with a refused default.
  ...
  @doc """
  Returns the rejection where `manifest` offers a template of refused shape.
  ...
  @spec validate_declaration(t()) :: problems()
  defp validate_declaration(manifest) do
    reports = [
  ...
  @spec validate_interior(t()) :: problems()
  defp validate_interior(manifest) do
    reports = [

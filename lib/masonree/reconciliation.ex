defmodule Masonree.Reconciliation do
  ...
  manifest declared, and writing `nil` would turn a never-decided attribute into
  one decided `nil` for every reader that asks. A value already held is never
  overwritten — a default is what a node holds in the absence of a decision, and
  a held value is a decision, whoever made it.
  ...
  @doc """
  Returns a finding for each attribute of `node` that would not store exactly.
  ...
  restring or refuse, is one finding naming the key.
  ...
  witness and then quietly become different data at the first write: an integer
  map key becomes a string, an atom key becomes a string, and an atom value
  becomes a string. A fourth shape does not restring, it refuses — a string
  carrying a NUL byte, because `jsonb` is `text` underneath and PostgreSQL has
  no representation for a NUL in `text`. Every other byte a binary can hold is
  fine — a tab, a newline, invalid UTF-8 — and only the NUL fails the write
  outright.
